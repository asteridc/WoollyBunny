...
public class DialogueManager : MonoBehaviour
{
    ...
    {
        int fallbackListIndex = chapters != null ? chapters.IndexOf(currentChapter) : -1;
        int stableChapterIndex = currentChapter != null && currentChapter.chapterIndex >= 0
            ? currentChapter.chapterIndex
            : fallbackListIndex;

        var data = new DialogueSaveData
        {
            chapterId = currentChapter != null ? currentChapter.chapterId : null,
            chapterIndex = stableChapterIndex,
    ...
    {
        currentChapter = ResolveChapterFromSave(data);
    ...
    {
        if (data == null || chapters == null || chapters.Count == 0)
            return null;
        ...
        if (!string.IsNullOrEmpty(data.chapterId))
        {
            for (int i = 0; i < chapters.Count; i++)
            {
                ...
                if (chapter != null && chapter.chapterId == data.chapterId)
                    return chapter;
            }
        }
        ...
        if (data.chapterIndex >= 0)
        {
            ...
            {
                var chapter = chapters[i];
                if (chapter != null && chapter.chapterIndex == data.chapterIndex)
                    return chapter;
            }
        }

        if (data.chapterIndex >= 0 && data.chapterIndex < chapters.Count)
