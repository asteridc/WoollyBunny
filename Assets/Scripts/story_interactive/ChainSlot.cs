...
public class ChainSlot : MonoBehaviour, IDropHandler
{
    ...
    public Image toolIconImage; // »конка инструмента в этом слоте
    ...
    public void EjectOtherIcons(ToolIcon keep)
    {
        var icons = GetComponentsInChildren<ToolIcon>(true);
        foreach (var icon in icons)
        {
            if (icon != null && icon != keep)
                icon.ReturnToOriginalSlot();
        }

        if (toolInSlot != null && toolInSlot != keep)
        {
            toolInSlot = null;
            currentTool = ToolType.None;
        }
